...
namespace llarp
{
...
#endif
  {
    ...
    {
      return out << italomq::to_hex(self.begin(), self.end());
    }
    ...
    ToHex() const
    {
      return italomq::to_hex(begin(), end());
    }

    std::string
    ShortHex() const
    {
      return italomq::to_hex(begin(), begin() + 4);
    }

    bool
    FromHex(std::string_view str)
    {
      if (str.size() != 2 * size() || !italomq::is_hex(str))
        return false;
      italomq::from_hex(str.begin(), str.end(), begin());
      return true;
    }
