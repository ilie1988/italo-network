...
Italonet es la implementación referente de LLARP (low latency anonymous routing protocol, protocolo de enrutado anónimo de baja latencia), un protocolo de enrutado onion de capa 3.
...
Puede ver la documentación, en ingles, de como empezar [aqui](https://italo-project.github.io/italo-docs/Italonet/ItalonetOverview/) .

![build status](https://gitlab.com/italoproject/italo-network/badges/master/pipeline.svg "build status")
![travis-ci](https://travis-ci.org/italo-project/italo-network.svg?branch=master "ci status")

## Uso

Vea, en ingles, [documentación](https://italo-project.github.io/italo-docs/Italonet/ItalonetOverview/) en como comenzar.

También lea, en ingles, [La guia de pruebas publicas](https://italodocs.com/Italonet/Guides/PublicTestingGuide/#1-italonet-installation) para la instalación y mas información util.
...
El instalador de MAC coloca los binarios normales (`italonet` y `italonet-bootstrap`) en `/usr/local/bin` los que pudieran estar en ruta, asi que usted puede usar los binarios facilmente desde su terminal. El instalador tambien truena sus configuraciones y llaves previas, y descarga la semilla de arranque mas actual.
...
para correr como usuario, correr el archivo en lote `run-italonet.bat` como su usuario normal.
...
compilando:
    ...
    $ git clone https://github.com/italo-project/italo-network
    $ cd italo-network
    $ mkdir build
    $ cd build
    $ cmake ..
...
compilar (donde `$ARCH` es su plataforma - `i686` or `x86_64`):
    ...
    $ git clone https://github.com/italo-project/italo-network.git
    $ cd italo-network
...
instalando (con priviligios elevados) en `$PROGRAMFILES/italonet` o `$ProgramFiles(x86)/italonet`:
